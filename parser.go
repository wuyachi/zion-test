...
func ParseExcel(excelPath string) {
	...
	c.ConsensusAddress = HDAddress{Index_1: 1, Index_2: 1}
	c.SignerAddress = HDAddress{Index_1: 1, Index_2: 2}
	c.ProposalAddress = HDAddress{Index_1: 1, Index_2: 3}
	c.Commission = 0
	c.InitStake = 0
	c.Desc = "validator 1"
...
func createRowAction(row []string) (action *RawAction, err error) {
	if len(row) == 0 {
		return
	}
	action = &RawAction{}
	...
	action.Sender = HDAddress{Index_1: uint32(index1), Index_2: uint32(index2)}
...
func NewComposeHandler(rawAction *RawAction) ComposeHandler {
